...
//     const tripData = tripSnap.data();
    ...
    // // 🕒 Calculate time since trip started
    // const startTime = new Date(tripData.createdAt);
    // const now = new Date();
    // const elapsedMs = now - startTime;
    // const elapsedSec = elapsedMs / 1000;

    // console.log(`⏱️ Trip running for ${Math.round(elapsedSec)} seconds`);
...
//     }
    ...
    // // 🚦 Demo signal logic — auto change after 68s, stay green for 8s, then red
    // const SIGNAL_DOC = "I01";
    // const signalRef = db.collection("intersectons").doc(SIGNAL_DOC);

    // let newStatus = "Red";
    // if (elapsedSec >= 68 && elapsedSec < 76) {
    //   newStatus = "Green";
    // }

    // await signalRef.set(
    //   {
    //     intersectionId: SIGNAL_DOC,
    ...
    // console.log(`🚦 Signal status: ${newStatus}`);
...
router.post("/updateLocation", async (req, res) => {
  try {
    ...
    console.log(`⏱️ Trip running for ${elapsedSec}s`);

    // Save new location
    await tripRef.update({ currentLocation: location, updatedAt: now });
    ...
    // 🚦 Demo-only logic — one intersection auto-toggles
    const SIGNAL_DOC = "I01"; // your Firestore doc ID for signal
    const signalRef = db.collection("intersectons").doc(SIGNAL_DOC);
    ...
    console.log(`🚦 Signal status set to: ${newStatus}`);

    res.json({ message: "Location updated", signalStatus: newStatus });
  } catch (err) {
    console.error("Error updating location:", err);
    res.status(500).json({ error: "Server error" });
  }
